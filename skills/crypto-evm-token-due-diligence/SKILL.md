...
1. **Provider policy.** Run `python3 "$SKILL_DIR/scripts/provider_context.py" --policy` and
   ...
   A dRPC key in the user's private env file is their standing authorization: source the
   env in the same shell call as every collector and pass `--provider auto --allow-network`;
   the backend uses dRPC when the key is present and the chain's
