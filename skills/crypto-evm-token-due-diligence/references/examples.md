...
| Exact-address discovery selects Robinhood mainnet; trusted local context records matching configured dRPC and standing bounded paid read-only permission | Source private exports in the same invocation, check with already-authorized flags, then collect chain ID, explicit header, runtime and final header recheck within a finite budget. Prefer this endpoint before public RPC. Prior successful evidence is not a new target pin. |
| Configured dRPC check returns `invocation_required`, with `network_disabled` and other omitted flags | Review local authorization once. If already granted, correct the invocation; if absent or revoked, use authorized alternatives without setup/payment prompts. No network request occurred, so this is not a provider failure. |
...
| Current user says no paid requests despite an older standing approval | Do not pass paid-use flags. Continue authorized alternatives; older context cannot override the current restriction. |
...
| Current process lacks dRPC variables but the user already saved the documented private env file | Load that file without printing values in the same invocation as the check/collector. Honor existing paid-use authorization; do not ask for the key again or label saved configuration missing. |
