...
class OptionalRpcTests(unittest.TestCase):
    def args(self, **changes):
        values = dict(allow_network=True, cost_policy="paid", allow_paid=True,
                      rpc_url_env="ROBINHOOD_DRPC_URL", provider="drpc", auth_env=None,
    ...
    def test_pre_rename_variable_is_named_so_the_fix_is_clear(self):
        # A private env still exporting the pre-rename CRYPTO_RPC_URL, with nothing under the current variable, is a
        # configuration gap that names the rename rather than a bare "unconfigured".
        result = self.check(self.args(allow_network=False, cost_policy=None, allow_paid=False),
                            {"CRYPTO_RPC_URL": "https://lb.drpc.live/robinhood", "DRPC_API_KEY": "TEST-SECRET"})
        self.assertEqual(result["reason"], "rpc_url_env_renamed")
        self.assertEqual(result["next_action"], "continue_standard_flow")
        self.assertNotIn("TEST-SECRET", json.dumps(result))
        # With the current variable set the hint does not fire.
        result = self.check(env={"ROBINHOOD_DRPC_URL": "https://lb.drpc.org/robinhood-mainnet", "CRYPTO_RPC_URL": "x", "DRPC_API_KEY": "TEST-SECRET"})
        self.assertEqual(result["status"], "ready")

    def test_malformed_endpoint_is_configuration_gap_even_without_flags(self):
        # A key inside the URL (a dkey parameter or a key path segment) is named so the fix is clear: move it to DRPC_API_KEY.
        for url in ("https://lb.drpc.live/?network=x&dkey=TEST-SECRET", "https://lb.drpc.live/robinhood/TEST-SECRET"):
            result = self.check(self.args(allow_network=False, cost_policy=None, allow_paid=False),
                                {"ROBINHOOD_DRPC_URL": url, "DRPC_API_KEY": "TEST-SECRET"})
            self.assertEqual(result["reason"], "rpc_url_carries_credential")
            self.assertEqual(result["reason_category"], "configuration")
            self.assertEqual(result["next_action"], "continue_standard_flow")
            self.assertFalse(result["provider_tested"])
            self.assertNotIn("TEST-SECRET", json.dumps(result))
        result = self.check(self.args(allow_network=False, cost_policy=None, allow_paid=False), {"ROBINHOOD_DRPC_URL": "http://lb.drpc.live/robinhood", "DRPC_API_KEY": "TEST-SECRET"})
    ...
    def test_omitted_flags_do_not_construct_transport_or_create_collection(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            stdout = io.StringIO()
            with patch.dict(os.environ, {"ROBINHOOD_DRPC_URL": "https://lb.drpc.live/robinhood-mainnet",
    ...
    def test_fresh_shell_must_source_exports_with_each_check(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            private = root/"env"
            private.write_text("export ROBINHOOD_DRPC_URL='https://lb.drpc.live/robinhood-mainnet'\n"
    ...
    def test_free_generic_provider_works_without_drpc_key(self):
        result = self.check(self.args(provider="generic", cost_policy="free", allow_paid=False),
                            {"ROBINHOOD_DRPC_URL": "https://public-rpc.example.invalid/"})
        self.assertEqual(result["status"], "ready")
    ...
    def test_invalid_configuration_reports_category_without_credentials(self):
        result = self.check(env={"ROBINHOOD_DRPC_URL": "https://lb.drpc.org/?network=ethereum&dkey=TEST-SECRET", "DRPC_API_KEY": "TEST-SECRET"})
        self.assertEqual(result["reason"], "rpc_url_carries_credential")
        self.assertNotIn("TEST-SECRET", json.dumps(result))
        result = self.check(env={"ROBINHOOD_DRPC_URL": "https://lb.drpc.org:99999/robinhood-mainnet", "DRPC_API_KEY": "TEST-SECRET"})
        self.assertEqual(result["reason"], "rpc_configuration_invalid")

    def test_default_generic_alias_detects_drpc_and_sends_key_only_as_header(self):
        args = self.args(provider="generic")
        env = {"ROBINHOOD_DRPC_URL": "https://lb.drpc.live/robinhood-mainnet", "DRPC_API_KEY": "TEST-SECRET"}
        self.assertEqual(self.check(args, env)["status"], "ready")
        with patch.dict(os.environ, env, clear=True):
            url, headers = rpc_collect.transport_settings(args)
        self.assertEqual(url, env["ROBINHOOD_DRPC_URL"])
    ...
    def test_malformed_port_and_header_credentials_fail_offline(self):
        for url, key in [("https://lb.drpc.live:bad/robinhood-mainnet", "TEST-SECRET"),
            ...
            with self.subTest(url=url):
                result = self.check(env={"ROBINHOOD_DRPC_URL": url, "DRPC_API_KEY": key})
                self.assertEqual(result["reason"], "rpc_configuration_invalid")
                self.assertEqual(result["network_requests"], 0)
                self.assertNotIn("TEST-SECRET", json.dumps(result))
    ...
    def test_budget_stop_does_not_request_an_alternate_provider(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            argv.extend(["--session", str(root / "session.sqlite")])
            stdout = io.StringIO()
            with patch.dict(os.environ, {"ROBINHOOD_DRPC_URL": "https://example.invalid/"}, clear=True), \
    ...
    def test_cli_missing_key_returns_distinct_handoff_without_artifacts(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            result = subprocess.run([sys.executable, str(SCRIPTS/"rpc_collect.py"), str(pp), "--out", str(root/"collection"),
                                    ...
                                    env={"ROBINHOOD_DRPC_URL": "https://lb.drpc.org/robinhood-mainnet", "PYTHONDONTWRITEBYTECODE": "1"},
