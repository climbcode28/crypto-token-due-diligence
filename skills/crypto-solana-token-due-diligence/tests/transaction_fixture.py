...
def fixture(version='legacy',wsol=False,closed=False,created=False,tip=0,buy=False,wrapped=0,wrap_source='owner'):
    """A sell of the target for a counter asset; `buy` swaps wrapped SOL for the target, and `wrapped` funds the
    input account in the same transaction (system transfer from `wrap_source`, then sync_native) before the leg."""
    ...
    inner=[ix(TOKEN_PROGRAM,[a['source'],a['input_mint'],a['input_vault'],a['owner']],struct.pack('<BQB',12,1000,9 if a['input_mint']==WSOL else 6),2),
           ix(TOKEN_PROGRAM,[a['output_vault'],a['output_mint'],a['destination'],a['authority']],struct.pack('<BQB',12,500,9 if a['output_mint']==WSOL else 6),2)]
