...
class RichRpc(Rpc):
    ...
    def reset(cls):
        ...
        cls.calls=[];cls.active=cls.peak=0;cls.mode='normal';cls.stamp=int(time.time());cls.receipt=None;return target
    def __call__(self,req):
        ...
        try:
            ...
            elif m=='getSignaturesForAddress':value=[{'signature':cls.receipt['transaction']['signatures'][0],'slot':100,'err':None,'memo':None,'blockTime':cls.stamp}] if cls.receipt else []
            elif m=='getTransaction':value=copy.deepcopy(cls.receipt)
            else:value=None
