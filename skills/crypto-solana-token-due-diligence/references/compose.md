...
`evidenced_external_limit` boundaries and no `pending_work`. The scaffold prefills every
row mechanically from the typed facts (`checked`/`resolved` when the surface's standard
...
`prefill.digest` still matches) is recomputed at each refresh (with the lane notes'
findings and checklists on disk) and at compose, so a preset or a lane result closes its
...
attributed (launch integrity then closes by your judgment on the sampled receipts or as
an evidenced limit). Finding and support
