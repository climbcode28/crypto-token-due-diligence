...
def decode(program,raw,accounts):
    ...
    p=source=dest=trader=None;vaults=[];mints=[];amount=threshold=None;mode=None
    ...
    return {'adapter':family[program], 'pool':p,'input_account':source,'output_account':dest,'trader':trader,
        'vaults':vaults,'mints':mints,'mode':mode,'specified_amount_atomic':str(amount),'threshold_atomic':str(threshold),
